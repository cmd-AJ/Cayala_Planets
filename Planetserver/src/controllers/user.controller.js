...
const actualizarDinos = async (req, res) => {
  try {
    ...
    if (!dinosValidos.includes(dinoNombre)) {
      return res.status(400).json({
        success: false,
        message: `El nombre del planeta '${dinoNombre}' no es válido`,
      });
    }
    ...
    if (todosDinosTrue) {
      ...
      console.log( "ID: "+ id + " Ha completado el juego")
    ...
    res.status(200).json({
      success: true,
      message: `El planeta '${dinoNombre}' fue actualizado exitosamente a true`,
      usuario,
    });
  } catch (error) {
...
const obtenerDinos = async (req, res) => {
  try {
    ...
    const dinos = {
      planeta_0: usuario.planeta_0,
      planeta_1: usuario.planeta_1,
      planeta_2: usuario.planeta_2,
      planeta_3: usuario.planeta_3,
      planeta_4: usuario.planeta_4,
      planeta_5: usuario.planeta_5,
      planeta_6: usuario.planeta_6,
      planeta_7: usuario.planeta_7,
    };
